...
def genetic(N: int, it: int, p: str):
    ...
    while population == None:
        population = initial_population(p, N-2)
    res_aprox_hpnx = apx.aproxHPNX(p)
    ind_to_coor_aprox_hpnx, coor_to_ind_aprox_hpnx, _ = apx.prot_coord(res_aprox_hpnx, in3D = True)
    population.append((res_aprox_hpnx, ind_to_coor_aprox_hpnx, coor_to_ind_aprox_hpnx))
    res_aprox_hp = apx_hp.algorithmC(apx_hp.format_seq(p), apx_hp.f)
    ind_to_coor_aprox_hp, coor_to_ind_aprox_hp, _ = apx.prot_coord(res_aprox_hp, in3D = True)
    population.append((res_aprox_hp, ind_to_coor_aprox_hp, coor_to_ind_aprox_hp))
    best_ins = []
    best_sol = 0
    results_ins = []
    results_sol = []
