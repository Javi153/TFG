...
def prot_fold(str_seq: str, algorithm: str, f = None):
    ...
    if algorithm == 'A' or algorithm == 'B':
        ...
        fit = fitness(str_seq, ind_to_coor, coor_to_ind, False)
        print('El valor obtenido por el algoritmo %s es' % algorithm, fit)
        ...
        fit = fitness(str_seq, ind_to_coor, coor_to_ind)
        print('El valor obtenido por el algoritmo C es', fit)
        """coord_x = [t[0] for t in coord]
        coord_y = [t[1] for t in coord] 
        ...
        plt.axis('equal')
        plt.show()"""
    return (ind_to_coor, coor_to_ind, coord), fit
