    ...
    if len(fold1) <= len(fold2):
        ...
        for i in range(0, len(fold2) - 1):
            if (p[fold1[len(fold1) - len(fold2) + i] + 1] == 'P' and p[fold2[len(fold2) - i - 2] + 1] == 'P') or (p[fold1[len(fold1) - len(fold2) + i] + 1] == 'N' and p[fold2[len(fold2) - i - 2] + 1] == 'N'):
                ...
                right_central[-2 * (i - 1)] = pts.Directions.B
    ...
    for i in range(len(p)):
        for n in neighbours(ind_to_coor[i], in3D):
            if coor_to_ind.get(n) != None and coor_to_ind.get(n) > i and coor_to_ind.get(n) - i > 1 and p[i] == 'H' and p[coor_to_ind.get(n)] == 'H':
                energy += 4
            elif coor_to_ind.get(n) != None and coor_to_ind.get(n) > i and coor_to_ind.get(n) - i > 1 and p[i] == 'P' and p[coor_to_ind.get(n)] == 'N':
                energy += 1
            elif coor_to_ind.get(n) != None and coor_to_ind.get(n) > i and coor_to_ind.get(n) - i > 1 and p[i] == 'N' and p[coor_to_ind.get(n)] == 'P':
                energy += 1
            elif coor_to_ind.get(n) != None and coor_to_ind.get(n) > i and coor_to_ind.get(n) - i > 1 and p[i] == 'P' and p[coor_to_ind.get(n)] == 'P':
                energy -= 1
            elif coor_to_ind.get(n) != None and coor_to_ind.get(n) > i and coor_to_ind.get(n) - i > 1 and p[i] == 'N' and p[coor_to_ind.get(n)] == 'N':
                energy -= 1
    return energy
...
def prot_fold(str_seq: str):
    ...
    print('El valor obtenido por el algoritmo HPNX es', fitness(str_seq, ind_to_coor, coor_to_ind))
